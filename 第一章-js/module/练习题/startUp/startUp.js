(function (global) {
	...
	function getDatamain() {
		var thisScript = document.getElementById("seajsnode");
		let thisurl = "";
		console.log("data-main:", thisScript.getAttribute("data-main"));
		if (thisurl = thisScript.getAttribute("data-main")) {
			data.cwd = data.cwd + thisurl + "/";
		};
		// console.log("data-main****", data.cwd);
	}

	startUp.use = function (list, callback) {
		// 检查有没有设置data-main
		// 
		// console.log("data.alias:", !data.alias || JSON.stringify(data.alias) == "{}");
		// console.log("data.paths:", data.paths);
		if ((!data.alias || JSON.stringify(data.alias) == "{}") && (!data.paths || JSON.stringify(data.paths) == "{}")) {
			getDatamain();
		}
		//检测有没有预先加载的模块  
